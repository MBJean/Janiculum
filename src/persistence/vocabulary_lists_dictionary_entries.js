...
module.exports = {
  async create(vocabularyListID, dictionaryEntryID) {
    ...
    await db.query(sql`
    INSERT INTO vocabulary_lists_dictionary_entries (vocablary_list_id, dictionary_entry_id)
  ...
  async delete(vocabularyListID, dictionaryEntryID) {
    await db.query(sql`
    DELETE FROM vocabulary_lists_organizations WHERE vocablary_list_id = ${vocabularyListID} AND dictionary_entry_id = ${dictionaryEntryID};
    `);
  }
};

...
module.exports = {
  async create(vocabularyListID, organizationID, admin = false) {
    ...
    await db.query(sql`
    INSERT INTO vocabulary_lists_organizations (vocablary_list_id, organization_id)
  ...
  async delete(vocabularyListID, organizationID) {
    await db.query(sql`
    DELETE FROM vocabulary_lists_organizations WHERE vocablary_list_id = ${vocabularyListID} AND organization_id = ${organizationID};
    `);
  }
};
